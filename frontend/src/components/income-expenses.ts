...
export class IncomeExpenses {
    ...
    private operationsContainer: HTMLElement | null;
    ...
    private async loadOperations(period: string = 'all', startDate: string | null = null, endDate: string | null = null): Promise<void> {
        try {
            let url: string = `/api/operations?period=${period}`;

            if (period === "interval" && startDate && endDate) {
                url += `&dateFrom=${startDate}&dateTo=${endDate}`;
            }

            const response: HttpResponseType<OperationsType[]> = await OperationService.getOperations(period, startDate, endDate);
    ...
    private renderOperations(operations: OperationsType[]): void {
        if (!this.operationsContainer) {
            return;
        }
        ...
        operations.forEach((operation: OperationsType) => {

            let savedCategory: { title: string } | null = null;
        ...
        <td class="text-center">${savedCategory?.title || "Категория не указана"}</td>
    ...
    private initEventListeners(): void {
        document.querySelectorAll(".delete-link").forEach((button) => {
            button.addEventListener("click", (event) => {
                ...
                const operationId: string | null = button.getAttribute("data-id");
                if (!operationId) {
                    return;
                }
    ...
    private initFilters(): void {
        ...
        if (dateRangeBox) {
            dateRangeBox.querySelectorAll("a").forEach((datePicker: HTMLAnchorElement, index: number) => {
                datePicker.addEventListener("click", () => {

                    const oldInput: HTMLElement | null = document.getElementById("date-picker");
    ...
    private showDeleteDialog(operationId: number | string): void {
        ...
        confirmDelete.onclick = async () => {
            await OperationService.deleteOperation(Number(operationId));

            const row = document.querySelector(`[data-id="${operationId}"]`)?.closest("tr");
